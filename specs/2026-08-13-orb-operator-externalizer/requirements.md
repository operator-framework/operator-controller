...
- `Externalize` returns the COD unchanged (with nil slices) when the serialized size is under the safe etcd threshold (900 KiB)
- `Externalize` converts all inline objects into COSLs and rewrites the COD when over the threshold
