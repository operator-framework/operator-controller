...
func TestObjectControllerManifests(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "cannot disable object-controller with Boxcutter", values: `options: {featureSet: experimental, objectController: {enabled: false}, operatorController: {features: {enabled: [BoxcutterRuntime], disabled: []}}}`, wantErr: "options.objectController.enabled=false is incompatible with enabled BoxcutterRuntime"},
		{name: "explicitly disabled with inactive Boxcutter", values: `options: {featureSet: experimental, objectController: {enabled: false}, operatorController: {features: {enabled: [BoxcutterRuntime], disabled: [BoxcutterRuntime]}}}`, wantOperatorController: true, wantCatalogd: true},
