...
func checkIfMatch(w http.ResponseWriter, r *http.Request) condResult {
	...
	for {
		...
		if etagStrongMatch(etag, getHTTPHeader(w.Header(), "Etag")) {
			return condTrue
		}
		im = remain
	}
...
func checkIfNoneMatch(w http.ResponseWriter, r *http.Request) condResult {
	inm := getHTTPHeader(r.Header, "If-None-Match")
	if inm == "" {
		return condNone
	}
	buf := inm
	for {
		...
		if etagWeakMatch(etag, getHTTPHeader(w.Header(), "Etag")) {
			return condFalse
		}
		buf = remain
	}
...
func checkIfRange(w http.ResponseWriter, r *http.Request, modtime time.Time) condResult {
	...
	ir := getHTTPHeader(r.Header, "If-Range")
	if ir == "" {
		return condNone
	}
	etag, _ := scanETag(ir)
...
// getHTTPHeader is like http.Header.Get, but key must already be in CanonicalHeaderKey form.
func getHTTPHeader(h http.Header, key string) string {
	if v := h[key]; len(v) > 0 {
		return v[0]
	}
	return ""
