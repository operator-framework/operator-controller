...
func reuseRollingOutRevision(ctx context.Context, state *reconcileState, ext *ocv1.ClusterExtension) bool {
	...
	anyHasDigest := false
	for _, rollingOut := range state.revisionStates.RollingOut {
	...
	if !anyHasDigest {
		// Backward compatibility for revisions created before digest tracking: continue
		// the in-progress rollout without catalog re-resolution when the rolling bundle
		// still satisfies the current spec version constraint. Apply backfills the digest
		// annotation on the next pass. If the spec changed (e.g. recovery from a failed
		// upgrade), re-resolve even without a stored digest.
		if versionMatchesSpec(latestRollingOut.Version, ext) {
			setRollingOutRevisionAsResolved(state, ext, latestRollingOut)
			return true
		}
		l.Info("no digest on rolling-out revisions and bundle version does not match spec, re-resolving bundle",
			"rollingOutVersion", latestRollingOut.Version,
...
// normalizeUpgradeConstraintPolicy maps an empty policy to CatalogProvided, matching
// the API default and resolution behavior in resolve/catalog.go.
func normalizeUpgradeConstraintPolicy(policy ocv1.UpgradeConstraintPolicy) ocv1.UpgradeConstraintPolicy {
	if policy == "" {
		return ocv1.UpgradeConstraintPolicyCatalogProvided
	}
	return policy
}
...
func CatalogSpecDigest(ext *ocv1.ClusterExtension) string {
	...
	data := struct {
		...
		UpgradeConstraintPolicy: normalizeUpgradeConstraintPolicy(cat.UpgradeConstraintPolicy),
	}
