...
func TestClusterExtensionBoxcutterApplierFailsDoesNotLeakDeprecationErrors(t *testing.T) {
	...
	specDigest := catalogSpecDigestFor(clusterExtension.Spec.Source.Catalog)

	cl, reconciler := newClientAndReconciler(t, func(d *deps) {
		// Boxcutter keeps a rolling revision when apply fails. We mirror that state so the test uses
		// the same inputs the runtime would see. The revision's CatalogSpecDigest must match the spec so
...
func TestClusterExtensionSpecMismatchWhileRollingOutReResolvesBundle(t *testing.T) {
	...
	oldSpecDigest := catalogSpecDigestFor(&ocv1.CatalogFilter{PackageName: "test-package", Version: "1.0.2"})
	...
	cl, reconciler := newClientAndReconciler(t, func(d *deps) {
		d.RevisionStatesGetter = &MockRevisionStatesGetter{
			RevisionStates: &controllers.RevisionStates{
				Installed: &controllers.RevisionMetadata{
					...
					Image:   "registry.example.com/test-bundle:v1.0.1-nginxolm88138",
					BundleMetadata: ocv1.BundleMetadata{
				...
				RollingOut: []*controllers.RevisionMetadata{{
					...
					Package:           "test-package",
					Image:             "registry.example.com/test-bundle:v1.0.2-test-package",
					CatalogSpecDigest: oldSpecDigest,
					BundleMetadata: ocv1.BundleMetadata{
						Name:    "test-package.v1.0.2",
						Version: "1.0.2",
					},
				}},
			},
		...
		d.Resolver = resolve.Func(func(ctx context.Context, ext *ocv1.ClusterExtension, installedBundle *ocv1.BundleMetadata) (*declcfg.Bundle, *bundle.VersionRelease, *declcfg.Deprecation, error) {
			...
			return &declcfg.Bundle{
				Name:    "test-package.v1.0.3",
				Package: "test-package",
				Image:   "registry.example.com/test-bundle:v1.0.3-nginxolm88138",
			}, &v, nil, nil
		})
		d.ImagePuller = &imageutil.MockPuller{ImageFS: fstest.MapFS{}}
		d.Applier = &MockApplier{installCompleted: true}
...
func TestClusterExtensionRollingOutRevisionMatchesSpecNoReResolve(t *testing.T) {
	...
	specDigest := catalogSpecDigestFor(&ocv1.CatalogFilter{PackageName: "test-package", Version: "1.0.2"})
...
func TestClusterExtensionMultipleRollingOutRevisionsUsesLatestMatch(t *testing.T) {
	...
	oldSpecDigest := catalogSpecDigestFor(&ocv1.CatalogFilter{PackageName: "test-package", Version: "1.0.2"})
	currentSpecDigest := catalogSpecDigestFor(&ocv1.CatalogFilter{PackageName: "test-package", Version: "1.0.1"})
	...
	cl, reconciler := newClientAndReconciler(t, func(d *deps) {
		d.RevisionStatesGetter = &MockRevisionStatesGetter{
			RevisionStates: &controllers.RevisionStates{
				Installed: &controllers.RevisionMetadata{
					Package: "test-package",
					Image:   "registry.example.com/test-bundle:v1.0.0-nginxolm88138",
					BundleMetadata: ocv1.BundleMetadata{
				...
				RollingOut: []*controllers.RevisionMetadata{
					...
					},
					{
						...
						Package:           "test-package",
						Image:             "registry.example.com/test-bundle:v1.0.1-nginxolm88138",
						CatalogSpecDigest: currentSpecDigest,
						BundleMetadata: ocv1.BundleMetadata{
							Name:    "test-package.v1.0.1",
							Version: "1.0.1",
						},
...
func TestClusterExtensionReResolvesWithInstalledAsFromVersion(t *testing.T) {
	...
	require.Equal(t, "test-package.v1.0.0", capturedInstalledBundle.Name,
...
func TestClusterExtensionRollingOutSameSpecHashReuses(t *testing.T) {
	...
	specDigest := catalogSpecDigestFor(&ocv1.CatalogFilter{PackageName: "test-package"})
...
// TestClusterExtensionRollingOutNoDigestSameSpecReuses verifies backward compatibility:
// revisions without a catalog spec digest continue rolling out when the bundle version
// still satisfies the current spec, without requiring catalog re-resolution.
func TestClusterExtensionRollingOutNoDigestSameSpecReuses(t *testing.T) {
	require.NoError(t, features.OperatorControllerFeatureGate.Set(fmt.Sprintf("%s=true", features.BoxcutterRuntime)))
	t.Cleanup(func() {
		require.NoError(t, features.OperatorControllerFeatureGate.Set(fmt.Sprintf("%s=false", features.BoxcutterRuntime)))
	})
	...
	require.False(t, resolverCalled,
		"resolver should NOT be called: legacy revision without digest still matches spec version")

	require.NoError(t, cl.DeleteAllOf(ctx, &ocv1.ClusterExtension{}))
}

// TestClusterExtensionRollingOutNoDigestSpecMismatchReResolves verifies that legacy
// revisions without a catalog spec digest still trigger re-resolution when the spec
// version changes during a stuck rollout (OCP-88138 / OCPBUGS-77829).
func TestClusterExtensionRollingOutNoDigestSpecMismatchReResolves(t *testing.T) {
	require.NoError(t, features.OperatorControllerFeatureGate.Set(fmt.Sprintf("%s=true", features.BoxcutterRuntime)))
	t.Cleanup(func() {
		require.NoError(t, features.OperatorControllerFeatureGate.Set(fmt.Sprintf("%s=false", features.BoxcutterRuntime)))
	})
		...
		d.RevisionStatesGetter = &MockRevisionStatesGetter{
			RevisionStates: &controllers.RevisionStates{
				...
				RollingOut: []*controllers.RevisionMetadata{{
					RevisionName: "extension-88138-2",
					Package:      "test-package",
					Image:        "registry.example.com/test-bundle:v1.0.2-nginxolm88138",
					BundleMetadata: ocv1.BundleMetadata{
						Name:    "test-package.v1.0.2",
						Version: "1.0.2",
					},
	...
	require.True(t, resolverCalled,
		"resolver should be called: legacy revision without digest but spec changed from v1.0.2 to v1.0.3")
...
func TestClusterExtensionRollingOutVersionRangeMatchReuses(t *testing.T) {
	...
	specDigest := catalogSpecDigestFor(&ocv1.CatalogFilter{PackageName: "test-package", Version: ">=1.0.0, <2.0.0"})
...
func TestCatalogSpecDigestSelectorNormalization(t *testing.T) {
	...
	t.Run("reordered MatchExpressions produce the same hash", func(t *testing.T) {
		...
		require.Equal(t, controllers.CatalogSpecDigest(makeExt(sel1)), controllers.CatalogSpecDigest(makeExt(sel2)))
	})
	...
	t.Run("different selectors produce different hashes", func(t *testing.T) {
		...
		require.NotEqual(t, controllers.CatalogSpecDigest(makeExt(sel1)), controllers.CatalogSpecDigest(makeExt(sel2)))
	})

	t.Run("nil selector is stable", func(t *testing.T) {
		h1 := controllers.CatalogSpecDigest(makeExt(nil))
		h2 := controllers.CatalogSpecDigest(makeExt(nil))
		require.Equal(t, h1, h2)
		require.NotEmpty(t, h1)
	})

	t.Run("nil and empty selector produce the same hash", func(t *testing.T) {
		hNil := controllers.CatalogSpecDigest(makeExt(nil))
		hEmpty := controllers.CatalogSpecDigest(makeExt(&metav1.LabelSelector{}))
		require.Equal(t, hNil, hEmpty, "nil and empty selectors are semantically equivalent and must hash identically")
	})

	t.Run("reordered channels produce the same hash", func(t *testing.T) {
		...
		ext2 := &ocv1.ClusterExtension{
			Spec: ocv1.ClusterExtensionSpec{
				Source: ocv1.SourceConfig{
					Catalog: &ocv1.CatalogFilter{
						PackageName: "test-package",
						Channels:    []string{"alpha", "stable", "beta"},
					},
				},
			},
		}
		require.Equal(t, controllers.CatalogSpecDigest(ext1), controllers.CatalogSpecDigest(ext2))
	})
}
...
// TestClusterExtensionVersionMismatchDuringRolloutReResolves verifies that when a
// ClusterExtension is rolling out version 1.0.2 (BadImage), and the user changes the spec
// to request version 1.2.0, the controller re-resolves the bundle instead of staying stuck.
func TestClusterExtensionVersionMismatchDuringRolloutReResolves(t *testing.T) {
	require.NoError(t, features.OperatorControllerFeatureGate.Set(fmt.Sprintf("%s=true", features.BoxcutterRuntime)))
	t.Cleanup(func() {
		require.NoError(t, features.OperatorControllerFeatureGate.Set(fmt.Sprintf("%s=false", features.BoxcutterRuntime)))
	})
	...
	cl, reconciler := newClientAndReconciler(t, func(d *deps) {
		d.RevisionStatesGetter = &MockRevisionStatesGetter{
			RevisionStates: &controllers.RevisionStates{
				RollingOut: []*controllers.RevisionMetadata{{
					RevisionName:      "extension-stuck-2",
					Package:           "test-package",
					Image:             "registry.example.com/test-bundle:v1.0.2-badimage",
					CatalogSpecDigest: oldSpecDigest,
					BundleMetadata: ocv1.BundleMetadata{
						Name:    "test-package.v1.0.2",
						Version: "1.0.2",
		...
		d.Resolver = resolve.Func(func(ctx context.Context, ext *ocv1.ClusterExtension, installedBundle *ocv1.BundleMetadata) (*declcfg.Bundle, *bundle.VersionRelease, *declcfg.Deprecation, error) {
			resolverCalled = true
			v := bundle.VersionRelease{
				Version: bsemver.MustParse("1.2.0"),
			}
			return &declcfg.Bundle{
				Name:    "test-package.v1.2.0",
				Package: "test-package",
				Image:   "registry.example.com/test-bundle:v1.2.0",
			}, &v, nil, nil
		})
		d.ImagePuller = &imageutil.MockPuller{ImageFS: fstest.MapFS{}}
		d.Applier = &MockApplier{installCompleted: true}
	...
	require.True(t, resolverCalled,
		"resolver should be called because the rolling-out revision (v1.0.2) does not match the spec (v1.2.0)")
	...
	require.NotNil(t, installedCond)
	require.Equal(t, metav1.ConditionTrue, installedCond.Status)
	require.Contains(t, installedCond.Message, "1.2.0")
