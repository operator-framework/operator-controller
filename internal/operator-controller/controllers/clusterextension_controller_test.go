...
func TestClusterExtensionBoxcutterApplierFailsDoesNotLeakDeprecationErrors(t *testing.T) {
	...
	cl, reconciler := newClientAndReconciler(t)
	// Boxcutter keeps a rolling revision when apply fails, so mimic that state to match runtime behavior.
	reconciler.RevisionStatesGetter = &MockRevisionStatesGetter{
	...
	t.Log("When the Boxcutter runtime is enabled and apply fails")
	clusterExtension := &ocv1.ClusterExtension{
		ObjectMeta: metav1.ObjectMeta{Name: extKey.Name},
		Spec: ocv1.ClusterExtensionSpec{
			Source: ocv1.SourceConfig{
	...
	require.Equal(t, metav1.ConditionFalse, packageCond.Status)
