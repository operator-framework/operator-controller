...
// and provide a bundle type. If an error is returned, the result is indeterminate.
func IsBundleSourceChart(bundleFS fs.FS) (bool, error) {
	chartPath := findChartArchive(bundleFS)
	if chartPath == "" {
		return false, nil
	}

	f, err := bundleFS.Open(chartPath)
	if err != nil {
		return false, err
	}
	...
	return len(c.Templates) > 0 && c.Metadata != nil, nil
}
