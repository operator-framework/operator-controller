...
func opconTweaks(channel string, name string, jsonProps apiextensionsv1.JSONSchemaProps) *apiextensionsv1.JSONSchemaProps {
	if strings.Contains(jsonProps.Description, "<opcon:validateIPAddress>") {
		jsonProps.Items.Schema.OneOf = []apiextensionsv1.JSONSchemaProps{{
			Properties: map[string]apiextensionsv1.JSONSchemaProps{
				"type": {
					Enum: []apiextensionsv1.JSON{{Raw: []byte("\"IPAddress\"")}},
				},
				"value": {
...
func formatDescription(description string, channel string, name string) string {
	...
	// Comments within "opcon:util:excludeFromCRD" tag is not included in the generated CRD and all trailing \n operators before
