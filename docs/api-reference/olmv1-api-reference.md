...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `namespace` _string_ | namespace references an existing namespace where namespace-scoped resources<br />for the extension are applied. The namespace must already exist on the cluster.<br /><opcon:standard:description><br />namespace is required.<br /></opcon:standard:description><br /><opcon:experimental:description><br />namespace is optional. When omitted, operator-controller resolves and creates a<br />managed namespace from bundle metadata. The mode (set vs omitted) is locked at<br />creation time and cannot be changed. Omitting namespace requires the experimental<br />feature set (BoxcutterRuntime).<br /></opcon:experimental:description><br />The namespace field follows the DNS label standard as defined in [RFC 1123].<br />It must contain only lowercase alphanumeric characters or hyphens (-), start and end with an alphanumeric character,<br />and be no longer than 63 characters.<br />[RFC 1123]: https://tools.ietf.org/html/rfc1123<br /><opcon:standard:validation:Required><br /><opcon:experimental:validation:XValidation:rule="oldSelf != '' \|\| self == ''",message="namespace cannot be set after creation; mode is locked at creation time"> |  | MaxLength: 63 <br />Optional: \{\} <br /> |
