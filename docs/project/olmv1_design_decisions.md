...
OLMv0’s primary use case encouraged cluster-admins to delegate extension management to namespace admins. OLMv0 runs as cluster-admin and exposes namespace-scoped APIs (`Subscription`, `OperatorGroup`) that allow non-admin users to trigger extension installations. This is a significant security issue: a namespace-scoped API causes cluster-scoped resources to be installed (CRDs, ClusterRoles, ClusterRoleBindings), meaning a namespace admin can effect cluster-wide changes through OLMv0’s cluster-admin credentials. OLMv0 has optional security controls — the OperatorGroup allows setting a ServiceAccount for authorization purposes, and cluster administrators can lock down CatalogSource usage and disable default catalogs — but if a cluster admin is not aware of these options, the default configuration results in any user with permission to create a Subscription effectively having cluster-admin. Because OLMv0 is used to install RBAC and run arbitrary workloads, there are numerous potential vectors that attackers could exploit. While there are no known exploits and there has not been any specific concern reported from customers, we believe CNCF’s reputation rests on secure cloud-native software and that this is a non-negotiable area to improve.

#### Evolution of OLM v1’s security model

OLM v1 has always expected only cluster-admins to interact with `ClusterExtension` and `ClusterCatalog` resources, eliminating OLMv0’s core security flaw: there is no namespace-scoped API that can trigger cluster-scoped changes.

OLM v1 initially required each ClusterExtension to reference a ServiceAccount for extension lifecycle operations. This was a **policy** mechanism — not a security boundary — since any user who could create a ClusterExtension could reference any existing ServiceAccount on the cluster. The intent was to give cluster-admins a way to scope the permissions available to each extension’s lifecycle operations. In practice, this model imposed significant operational burden: users had to derive and maintain complex RBAC for every extension installation, and the permission requirements changed on every upgrade. The policy value did not justify the complexity cost.

OLM v1 now uses a simpler model:

- OLM v1’s operator-controller runs as cluster-admin. The security boundary is Kubernetes RBAC on OLM’s own APIs. `ClusterExtension` and `ClusterCatalog` are cluster-scoped resources that Kubernetes RBAC restricts to cluster-admins by default. Creating a `ClusterExtension` is an explicitly cluster-admin operation — the privilege boundary and the API boundary are the same.
- OLM v1 will use secure communication protocols between all internal components and between itself and its clients.

This change does mean that cluster-admins who relied on per-extension ServiceAccounts as a policy tool lose that capability within OLM itself. However, Kubernetes already provides native mechanisms for this kind of policy enforcement through its admission validation extension points. For example, `ValidatingAdmissionPolicy`, validating admission webhooks, or policy engines such as Kyverno and OPA Gatekeeper can restrict what resources may be created with an `ownerReference` pointing to a particular `ClusterExtension`, giving cluster-admins fine-grained control over what each extension is allowed to install without requiring OLM to reimplement that policy layer.
