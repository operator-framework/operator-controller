...
func BeforeSuite() {
	...
	raw, err := k8scli("get", "deployment", "-n", olmNamespace, olmDeploymentName, "-o", "json")
	if err != nil {
		panic(fmt.Errorf("failed to get OLM deployment: %v", err))
	}
	d := &v1.Deployment{}
...
func ScenarioCleanup(ctx context.Context, _ *godog.Scenario, err error) (context.Context, error) {
	...
	go func() {
		for _, r := range forDeletion {
			if _, err := k8scli("delete", r.kind, r.name, "--ignore-not-found=true"); err != nil {
