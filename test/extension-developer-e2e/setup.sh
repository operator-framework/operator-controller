...
- \$INCLUSTER_REGISTRY_HOST - hostname:port of the local docker-registry
...
if [[ -z "${INCLUSTER_REGISTRY_HOST}" ]]; then
  echo "\$INCLUSTER_REGISTRY_HOST is required to be set"
  echo "${help}"
  exit 1
fi
...
reg_bundle_img="${INCLUSTER_REGISTRY_HOST}/bundles/registry-v1/registry-bundle:v0.0.1"
