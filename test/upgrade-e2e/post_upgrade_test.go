...
func findLeaderPod(ctx context.Context, controlPlaneLabel string) (*corev1.Pod, error) {
	...
	err := wait.PollUntilContextTimeout(ctx, 2*time.Second, pollDuration, true, func(ctx context.Context) (bool, error) {
		// Fetch lease first to avoid race condition where pod count changes between list and lease check
		...
		// The HolderIdentity is typically in the format "pod-name_hash"
		...
		// Find the pod that matches the holder identity
		for i := range managerPods.Items {
			pod := &managerPods.Items[i]
			// The holder identity contains the pod name as a prefix
			if strings.HasPrefix(holderIdentity, pod.Name+"_") {
				leaderPod = pod
				return true, nil
			}
		}

		// If we couldn't match the holder identity to a pod, retry
		// This can happen during pod rotation
		return false, nil
	})
